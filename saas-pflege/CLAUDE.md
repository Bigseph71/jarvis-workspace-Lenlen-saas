...
1. **Suppression douce uniquement.** `deleted_at`, `deletion_reason`,
   ...
   conservation (§ 630f BGB). La suppression coupe la connexion (`auth.service`
   écarte les organisations supprimées) et passe le statut à `CANCELED` — sans
   quoi elle ne serait qu'un changement d'affichage.
